...
(async () => {
	...
	const packageManager = parseZodSchema(z.enum(PackageManager), getOptionalInput("package_manager") ?? PackageManager.NPM);
	const dependencyGroup = parseZodSchema(z.enum(DependencyGroup), getOptionalInput("dependency_group") ?? DependencyGroup.DEPENDENCIES);
	const strict = getBooleanInput("strict_version_resolution");
	const dependencies = getDependenciesFromGroup(await getPackageJsonContents(process.cwd(), { strict }) ?? {}, dependencyGroup);
